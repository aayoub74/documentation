...
<email_servers_outbound/notification_system>` `notifications@company-name.odoo.com`.
...
:ref:`new domain alias <email_servers_outbound/alias_domain>` page allowing to configure the alias
of your companies. If only one domain is configured, this domain will be shared by all companies
on the database.
...
If :ref:`you want to use your own mail server <email_servers_outbound/external_smtp>`, you will
...
- For incoming emails, set the frequency at which you fetch for new emails low enough for responsiveness
  but high enough in order not to stress your system or provider. Due to this reason and the simplicity
  of this configuration, we usually advice on using incoming mail servers. To use an SMTP server,
  check out the :ref:`"Use a custom domain for inbound messages" documentation
